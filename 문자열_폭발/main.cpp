...
int main()
{
    ...
    stack<int> s;
    vector<int> check(input.length());
    int streak = 0, word_idx = word.length() - 1;
    for (int i = input.length() - 1; i >= 0; --i)
    {
        if (!s.empty() && streak == word.length() - word_idx && input[s.top()] == word[word_idx])
        {
            if (word_idx == 0)
            {
                while (!s.empty())
