...
int main()
{
    ...
    for (int i = n - 1; i >= 0; --i)
    {
        while (!stk.empty() && appear[arr[i]] > appear[stk.back()])
        {
            result[i] = stk.back();
            stk.pop_back();
        }
